...
let check_param_eq m ml =
  let param_eq b p1 p2 =
    match p1, p2 with
    | Pvar v1, Pvar v2 -> b && ty_eq v1.v_ty v2.v_ty
    | Plabel l1, Plabel l2 -> b
    | _, _ -> false in
  try
...
let rec traverse_code genv instrs =
  ...
  | i::is ->
    begin
      ...
      | Imacro(mname, margs) ->
        begin
          match (add_leakicall genv i.i_loc mname margs) with
